...
# set_the transformer outputs to pandas
...
target_cols='time_taken'

# generate order for ordinal encoding
traffic_order=['low', 'medium', 'high', 'jam']
distance_type_order=['short', 'medium', 'long', 'verylong']
...
def drop_missing_values(data: pd.DataFrame) -> pd.DataFrame:
    logger.info(f"Original dataset: {data.shape[0]} rows, {data.shape[1]} cols")
    df_dropped = data.dropna()
    logger.info(f"After dropna: {df_dropped.shape[0]} rows, {df_dropped.shape[1]} cols")

    if df_dropped.empty:
        raise ValueError("All rows dropped — dataset unusable under strict dropna rule.")
    return df_dropped
...
if __name__=='__main__':
    ...
    # split the train and test data with missing values dropped
    X_train, y_train=make_X_and_y(data=train_df, target_column=target_cols)
    X_test, y_test=make_X_and_y(data=test_df, target_column=target_cols)
    logger.info('Data splitting completed')

    # fit the preprocesor on X_train
