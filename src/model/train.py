...
# create logger
logger=logging.getLogger('model_traning')
...
def load_data(data_path: Path)->pd.DataFrame:
    try:
        ...
        logger.error('This error is occurred {e}')
    return df
...
if __name__=='__main__':
    ...
    logger.info('Training Data read Successful')
    ...
    logging.info('Target Transformer built')

    # from the stacking regressor
    stacking_reg=StackingRegressor(
        estimators=[('rf_model', rf),
                    ('lgbm_model', lgbm)],
                    final_estimator=lr,
                    cv=5, n_jobs=-1)
